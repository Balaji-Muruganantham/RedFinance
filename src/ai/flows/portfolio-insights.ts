...
const AnalyzePortfolioInputSchema = z.object({
  holdings: z
    .string()
    .describe(
      'A list of investment holdings with ticker symbols and quantities.'
    ),
  transactionHistory: z
    .string()
    .describe(
      'A record of all transactions, including purchases and sales, with dates and amounts.'
    ),
  assetAllocation: z
    .string()
    .describe(
      'The portfolio asset allocation across different investment types and sectors.'
    ),
...
const AnalyzePortfolioOutputSchema = z.object({
  insights: z
    .string()
    .describe(
      'Personalized insights and analysis of portfolio performance, highlighting key trends and opportunities.'
    ),
  riskFactors: z
    .string()
    .describe('Identified risk factors within the portfolio.'),
  opportunities: z
    .string()
    .describe('Potential opportunities for portfolio improvement.'),
});
export type AnalyzePortfolioOutput = z.infer<
  typeof AnalyzePortfolioOutputSchema
>;
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a financial advisor providing insights on investment portfolios in the Indian market.

  Analyze the following portfolio data to identify key trends, risk factors, and potential opportunities for improvement. The currency is in Indian Rupees (INR).
